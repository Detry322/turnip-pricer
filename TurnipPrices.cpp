...
public:
  ...
  uint32_t getU32();
  uint64_t getU64();
  void getContext(uint32_t *seed1, uint32_t *seed2, uint32_t *seed3,
                  uint32_t *seed4) const;

private:
  uint32_t mContext[4];
};

void Random::init() { init(42069); }
...
void Random::init(uint32_t seed1, uint32_t seed2, uint32_t seed3,
                  uint32_t seed4) {
...
uint64_t Random::getU64() {
  ...
  mContext[3] =
      n2 ^ (n2 << 11) ^ ((n2 ^ (n2 << 11)) >> 8) ^ mContext[2] ^ (n3 >> 19);

  return ((uint64_t)mContext[2] << 32) | mContext[3];
}

void Random::getContext(uint32_t *seed1, uint32_t *seed2, uint32_t *seed3,
                        uint32_t *seed4) const {
  *seed1 = mContext[0];
  *seed2 = mContext[1];
  *seed3 = mContext[2];
...
uint32_t pf(float f) { return *((uint32_t *)&f); }
struct TurnipPrices {
  int32_t basePrice;
...
int main(int argc, char **argv) {
  ...
  printf("     %3d  %3d  %3d  %3d  %3d  %3d\n", turnips.sellPrices[3],
         turnips.sellPrices[5], turnips.sellPrices[7], turnips.sellPrices[9],
         turnips.sellPrices[11], turnips.sellPrices[13]);
